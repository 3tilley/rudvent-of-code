...
fn next_index(
    ...
    let first = (
        current_index
            .0
            .checked_add_signed(piece.0 .0 as isize)
            .unwrap(),
        current_index
            .1
            .checked_add_signed(piece.0 .1 as isize)
            .unwrap(),
    );
    let second = (
        current_index
            .0
            .checked_add_signed(piece.1 .0 as isize)
            .unwrap(),
        current_index
            .1
            .checked_add_signed(piece.1 .1 as isize)
            .unwrap(),
    );
...
pub fn prepare(input: String) -> InputPart1 {
    ...
    let map =
        array2d::Array2D::from_iter_row_major(&mut elements.into_iter().flatten(), rows, cols)
            .unwrap();
    Map { map, rows, cols }
}
...
pub fn part_1(
    ...
    let s = input
        .map
        .enumerate_row_major()
        .find(|((row_col), char)| **char == 'S')
        .unwrap();
    let mut previous = s.0;
    let touching = &[(0, -1), (1, 0), (1, 0), (0, -1)]
        .iter()
        .filter_map(|offset| {
            let new_row = previous
                .0
                .checked_add_signed(offset.0)
                .and_then(|row| (row < input.rows).then_some(row));
            let new_col = previous
                .1
                .checked_add_signed(offset.1)
                .and_then(|col| (col < input.cols).then_some(col));
