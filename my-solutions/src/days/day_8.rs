...
pub fn prepare(input: String) -> InputPart1 {
    ...
    let map = lines
        .map(|line| {
            ...
            (
                str_to_tuple(left.trim()),
                (str_to_tuple(tuple.0.trim()), str_to_tuple(tuple.1.trim())),
            )
        })
        .collect();
    Map { seq, links: map }
...
pub fn part_2(
    ...
    let mut currents = input
        .links
        .iter()
        .filter_map(|link| ends_with_a(link.0).then_some((*link.0, *link.1)))
        .collect::<Vec<_>>();
    let cycle = input.seq.chars().cycle();
    let mut steps = 0;
    for step in cycle {
        ...
        currents = currents
            ...
            .map(|link| {
                // info!("{:?}", next);
                let next = if step == 'L' { link.1 .0 } else { link.1 .1 };
                (next, *input.links.get(&next).unwrap())
            })
            .collect();
        if currents.iter().all(|link| ends_with_z(&link.0)) {
            break;
        }
    }
    return steps;
