...
const EXAMPLE_1_ANS: OutputPart1 = 1320;
...
pub fn prepare(input: String) -> InputPart1 {
    input.split(",").map(Word::new).collect()
}
...
pub fn part_1(
    ...
    input.iter().map(|word| word.advent_hash() as usize).sum()
}
